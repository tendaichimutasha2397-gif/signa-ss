...
// Fallback stock source: Yahoo Finance's public chart endpoint (free, no
// key). Used only when Stooq fails or returns nothing — two independent
// free sources are more resilient than one, and this app should say
// clearly when it's had to fall back rather than silently mixing them.
...
async function fetchStockHistory(stooqSym) {
  try {
    ...
    // Fall back to Yahoo rather than surfacing an error the user can't act on.
    const plainSym = stooqSym.replace(/\.us$/i, '').toUpperCase();
    const yahoo = await fetchStockHistoryYahoo(plainSym);
    // Reshape to match Stooq's `rows` shape (used elsewhere for prevClose/volume).
    const rows = yahoo.closes.map((c, i) => ({ close: c, high: yahoo.highs[i], low: yahoo.lows[i], volume: yahoo.volumes[i] }));
    return { ...yahoo, rows };
  }
}
...
async function getSnapshot(symbols) {
  ...
  await Promise.all(knownStock.map(async (sym) => {
    ...
    try {
      ...
      out[sym] = { ok: false, reason: `${e.message} (tried Stooq and Yahoo fallback — both unavailable right now; free data can be delayed/unavailable outside US market data licensing)`, symbol: sym, type: 'stock' };
    }
  }));
...
async function getHistory(symbol) {
  ...
  try {
    if (entry.type === 'crypto') {
      ...
      } catch { /* highs/lows optional for backtesting extra metrics */ }
...
// Fetches closing-price history for several symbols in parallel — used by
// the correlation matrix. Best-effort: a symbol that fails is simply
