...
function load() {
  try {
    ...
    return { items: [], seenIds: {}, watchlist: DEFAULT_WATCHLIST, extraKeywords: [], trackedAssets: DEFAULT_ASSETS, tvSymbols: DEFAULT_TV_SYMBOLS, alerts: [], lastIndicatorState: {} };
  }
}
...
// Free-form TradingView symbols (e.g. "NASDAQ:AAPL", "BINANCE:BTCUSDT",
// "FX:EURUSD") — separate from trackedAssets above, which is the fixed
// CoinGecko/Stooq-backed list. This list isn't validated server-side since
// TradingView itself resolves (or rejects) whatever symbol string it's given.
function getTvSymbols() {
  return state.tvSymbols;
}

function setTvSymbols(list) {
  state.tvSymbols = list;
  save();
}
...
module.exports = {
  ...
  getTrackedAssets, setTrackedAssets, getTvSymbols, setTvSymbols, addAlert, getAlerts, getLastIndicatorState, setLastIndicatorState,
};
